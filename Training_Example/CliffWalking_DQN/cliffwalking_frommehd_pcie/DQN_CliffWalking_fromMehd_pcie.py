...
# parser.add_argument("--seed", type=int, default=2024, help="Random seed")  # 新增的命令行参数
...
os.pwrite(pc2fpga, buffer, 0x0000)
read_num = para_num * 4
read_offset = para_num * 4 + 4
...
class DQN_Agent:
    ...
    #     optimized to use numpy operations and minimize type conversions.

    #     Parameters:
    #         state (torch.Tensor): Input tensor representing the state.

    #     Returns:
    #         actions (numpy.ndarray): The selected actions as a uint32 array.
    #     """

    #     # Generate random actions for all environments first
    #     actions = np.random.randint(0, 4, size=self.env_num, dtype=np.uint32)

    #     # Exploitation: the action is selected based on the Q-values.
    #     # Only compute Q-values if exploitation is chosen based on epsilon threshold
    #     if np.random.random() >= self.epsilon_max:
    #         with torch.no_grad():
    #             Q_values = self.main_network(state)
    #             torch_actions = torch.argmax(Q_values, dim=1)  # get actions from the network
    #             actions = torch_actions.cpu().numpy().astype(np.uint32)  # convert to numpy uint32 only once
    ...
    def select_action(self, state):
        ...
        if np.random.random() < self.epsilon_max:
            sampled_actions = np.random.randint(0, 4, size=self.env_num)
            return np.array(sampled_actions, dtype=np.int32)
...
class Model_TrainTest:
    ...
    def state_preprocess(self, state, num_states=48):
        state_tensor = torch.tensor(state.astype(np.int64), dtype=torch.int64)
        onehot_encoded = torch.nn.functional.one_hot(state_tensor, num_classes=num_states)
        return onehot_encoded.float()

    def parse_data(self, fpga_raw):
        data_array = np.frombuffer(fpga_raw, dtype=np.uint32)
        next_state_int = (data_array >> 16)
        # print("next_state_int_1", next_state_int)
        next_state_int = next_state_int.astype(np.int32)
        # print("next_state_int_2", next_state_int)
        done = (data_array >> 8) & 0xFF
        reward = (data_array & 0xFF).astype(np.int32)
        reward = -reward
        # print("reward_parse", reward)
        return next_state_int, done, reward

    
    # def cliff_walking_step_vectorized(self, actions, states):
    #     next_states = []
    #     rewards = []
    #     dones = []
    ...
    #                 next_state = state + 12
    #         if action == 3:
    #             if (state == 0 or state == 12 or state == 24 or state == 36):
    #                 next_state = state
    #             else:
    #                 next_state = state - 1
                    
    #         if (next_state > 36 and next_state < 47):
    #             reward = -100
    #             done = 0
    #             next_state = 36
    #         elif (next_state == 47):
    ...
    def train(self):
        ...
        total_steps = 0
        # episode_reward = np.int32(0)  # 初始化为32位整数
        ...
        for episode in range(0, self.max_episodes):
            ...
            state_int = np.full(self.env_num, 36, dtype=np.int32)      
            state = self.state_preprocess(state_int, num_states=48)
            done = False
            truncation = False
            step_size = 0
            episode_reward = 0 # 初始化为32位整数

            while not np.any(done) and step_size < self.max_steps:
                # action_start_time = time.time_ns()
                action = self.agent.select_action(state)
                # action_end_time = time.time_ns()
                # print(f"action_time: {(action_end_time - action_start_time) / 1e9}s")
                # step_start_time = time.time_ns()
                # action = np.array(action, dtype=np.int32)
                combined_data = (state_int << 16) | action
                src_data = combined_data.tobytes()
                src_data = src_data + start_flag
                os.pwrite(pc2fpga, src_data, 0x0000)
                fpga_raw = os.pread(fpga2pc, read_num, read_offset) 
                next_state_int, done, reward= self.parse_data(fpga_raw)
                # step_end_time = time.time_ns()
                # print(f"step_time: {(step_end_time - step_start_time) / 1e9}s")


                # check_next_states, check_rewards, check_dones  = self.cliff_walking_step_vectorized (action, state_int)
                # diff_next_state = np.array(check_next_states) - next_state_int
                # diff_reward = np.array(check_rewards) - reward
                # diff_done = np.array(check_dones) -  done
                # if (diff_next_state != 0).any() | (diff_reward != 0).any() | (diff_done != 0).any():
                #     print("state", state)
                #     print("reward:", reward)
                #     print("done", done)
                #     print("action", action)
                #     print("next_state_int:", next_state_int)
                #     print("check_next_states", check_next_states)
                #     print("check_rewards", check_rewards)
                #     print("check_dones", check_dones)
                #     print("diff_next_state", diff_next_state)
                #     print("diff_reward", diff_reward)
                #     print("diff_done", diff_done)



                next_state = self.state_preprocess(next_state_int, num_states=48)
                ...
                # print(f"train_time: {(train_end_time - train_start_time) / 1e9}s")
...
with open(f'/data1/FPGA_GYM/python/CliffWalking/cliffwalking_frommehd_pcie/data/reward_buffer_envnum{ENV_NUM}_{curve_idx}.txt', 'w') as f:
    for reward in DRL.REWARD_BUFFER:
        f.write(str(reward) + '\n')
with open(f'/data1/FPGA_GYM/python/CliffWalking/cliffwalking_frommehd_pcie/data/time_buffer_envnum{ENV_NUM}_{curve_idx}.txt', 'w') as f:
    for this_time in DRL.TIME_BUFFER:
        f.write(str(this_time) + '\n')
with open(f'/data1/FPGA_GYM/python/CliffWalking/cliffwalking_frommehd_pcie/data/ave_reward_buffer_envnum{ENV_NUM}_{curve_idx}.txt', 'w') as f:
...
plt.grid(True)
plt.savefig(f'/data1/FPGA_GYM/python/CliffWalking/cliffwalking_frommehd_pcie/data/reward_episode_envnum{ENV_NUM}_{curve_idx}.png')
...
plt.grid(True)
plt.savefig(f'/data1/FPGA_GYM/python/CliffWalking/cliffwalking_frommehd_pcie/data/reward_time_envnum{ENV_NUM}_{curve_idx}.png')
...
plt.grid(True)
plt.savefig(f'/data1/FPGA_GYM/python/CliffWalking/cliffwalking_frommehd_pcie/data/ave_reward_time_envnum{ENV_NUM}_{curve_idx}.png')
...
plt.grid(True)
plt.savefig(f'/data1/FPGA_GYM/python/CliffWalking/cliffwalking_frommehd_pcie/data/ave_reward_episodes_envnum{ENV_NUM}_{curve_idx}.png')
