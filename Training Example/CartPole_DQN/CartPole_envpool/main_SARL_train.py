...
if __name__ == "__main__":
    ...
    envs = envpool.make('CartPole-v1',  env_type="gym", num_envs= ENV_NUM)
    s, info = env.reset()
    n_state = len(s)
    ...
    for episode_i in range(n_episode):
        ...
        for step_i in range(n_time_step):
            ...
            if random_sample <= epsilon:
                a = np.random.randint(0, 2, size=ENV_NUM)
            ...
            # start_time = time.time_ns()
            s_, r, done, _, _ = envs.step(a)  # timelimit and info are not used in this case
...
with open(f'/data0/FPGA_GYM/python/CartPole/CartPole_envpool/data/reward_buffer_envnum{ENV_NUM}_{curve_idx}.txt', 'w') as f:
    for reward in REWARD_BUFFER:
        f.write(str(reward) + '\n')
with open(f'/data0/FPGA_GYM/python/CartPole/CartPole_envpool/data/time_buffer_envnum{ENV_NUM}_{curve_idx}.txt', 'w') as f:
    for this_time in TIME_BUFFER:
        f.write(str(this_time) + '\n')
with open(f'/data0/FPGA_GYM/python/CartPole/CartPole_envpool/data/ave_reward_buffer_envnum{ENV_NUM}_{curve_idx}.txt', 'w') as f:
...
plt.grid(True)
plt.savefig(f'/data0/FPGA_GYM/python/CartPole/CartPole_envpool/data/reward_episode_envnum{ENV_NUM}_{curve_idx}.png')
...
plt.grid(True)
plt.savefig(f'/data0/FPGA_GYM/python/CartPole/CartPole_envpool/data/reward_time_envnum{ENV_NUM}_{curve_idx}.png')
...
plt.grid(True)
plt.savefig(f'/data0/FPGA_GYM/python/CartPole/CartPole_envpool/data/ave_reward_time_envnum{ENV_NUM}_{curve_idx}.png')
...
plt.grid(True)
plt.savefig(f'/data0/FPGA_GYM/python/CartPole/CartPole_envpool/data/ave_reward_episodes_envnum{ENV_NUM}_{curve_idx}.png')
